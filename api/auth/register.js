...
export default async function handler(req, res) {
  ...
  // Buat akun di Supabase Auth pakai email dummy (username@nyzz.internal)
  ...
  if (profileErr) {
    // Rollback: hapus user auth kalau profile gagal dibuat
    await supabase.auth.admin.deleteUser(authData.user.id);
    return res.status(500).json({ error: 'Gagal membuat profil' });
  }

  // Sign in langsung untuk dapat session
